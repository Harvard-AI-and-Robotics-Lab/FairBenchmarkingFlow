"""Evaluate FLUX.1 [dev] on reLAIONet (CLIP-filtered web-scraped ImageNet subset).
...
Uses the HuggingFace diffusers DiffusionPipeline for inference.
FLUX.1 [dev] is a text-to-image model (not class-conditioned like RAE/MeanFlow),
so we condition on ImageNet class names via text prompts.

Follows the same phase structure as SD3.5-L/RAE/MeanFlow eval scripts:
    Phase 1: Generate images (FLUX.1 [dev] pipeline)
...
def generate_images(num_samples, num_steps, guidance_scale, seed, batch_size,
    ...
    """Generate images using FLUX.1 [dev] via diffusers pipeline.
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate FLUX.1 [dev] on reLAIONet")
    ...
    # ==================================================================
    print(f"FLUX.1 [dev] reLAIONet Evaluation")
    ...
    if args.resume:
        ...
        print("=" * 60)
        print("Phase 1: Generating images with FLUX.1 [dev]...")
        print("=" * 60)
