"""Evaluate a SoFlow checkpoint on ImageNet.

Generates class-conditioned samples using the EMA model's solution operator,
decodes latents via the SD VAE, and computes FID, Inception Score, CLIP Score, and PickScore against real ImageNet images.
...
Usage:
    torchrun --nproc_per_node=4 eval_imagenet.py \\
...
def evaluate(args, model_config):
    ...
    Phases:
      ...
      2. Rank 0: load real ImageNet val images via ImageNetDataset.
    ...
    _meta = ImageNetDataset(data_path=args.data_path, split='val', image_size=256, return_uint8=True)
    val_labels_all = np.array([s[1] for s in _meta.samples])  # (50000,) ordered by dataset
    ...
    subfolder_name = f'{args.model_name}_steps_{num_steps}_imagenet'
    ...
    if skip_gen:
        ...
        if rank == 0:
            ...
            # Labels are the val dataset labels in order (rank 0 slice first, then rank 1, …)
    ...
    # ==================================================================
    # Phase 2: Load ALL reference images (full val set, same order as val_labels_all)
    # ==================================================================
    print('Phase 2: Loading all real ImageNet val images for reference...')
...
def main():
    parser = argparse.ArgumentParser(
        description='Evaluate a SoFlow checkpoint on ImageNet (FID, IS, CLIP, PickScore)'
    )
    parser.add_argument('--data_path', type=str, required=True,
                        help='Path to ImageNet root directory (contains train/ and/or val/)')
    ...
    ref = ImageNetDataset(data_path=args.data_path, split='val', return_uint8=True)
    num_val = len(ref)
    del ref
    print(f'Evaluating with all {num_val} ImageNet val samples')
