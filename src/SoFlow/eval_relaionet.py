"""Evaluate a SoFlow checkpoint on reLAIONet (CLIP-filtered web-scraped ImageNet subset).

Generates class-conditioned samples using the EMA model's solution operator,
decodes latents via the SD VAE, and computes FID, Inception Score, CLIP Score, and PickScore against real reLAIONet images.
...
Usage:
    torchrun --nproc_per_node=4 eval_relaionet.py \\
...
def evaluate(args, model_config):
    ...
    Phases:
      ...
      2. Rank 0: load real reLAIONet images via ReLAIONetDataset.
    ...
    _meta = ReLAIONetDataset(data_path=args.data_path, image_size=256, return_uint8=True)
    val_labels_all = np.array([s[1] for s in _meta.samples])  # ordered by dataset
    ...
    subfolder_name = f'{args.model_name}_steps_{num_steps}_relaionet'
    ...
    if skip_gen:
        ...
        if rank == 0:
            ...
            # Labels are the dataset labels in order (rank 0 slice first, then rank 1, …)
    ...
    # ==================================================================
    # Phase 2: Load ALL reference images (reLAIONet)
    # ==================================================================
    print('Phase 2: Loading all real reLAIONet reference images...')
...
def main():
    parser = argparse.ArgumentParser(
        description='Evaluate a SoFlow checkpoint on reLAIONet (FID, IS, CLIP, PickScore)'
    )
    parser.add_argument('--data_path', type=str, required=True,
                        help='Path to reLAIONet root directory (contains reLAIONet-cleaned-v1/)')
    ...
    ref = ReLAIONetDataset(data_path=args.data_path, return_uint8=True)
    num_relaionet = len(ref)
    del ref
    print(f'Evaluating with all {num_relaionet} reLAIONet samples')
