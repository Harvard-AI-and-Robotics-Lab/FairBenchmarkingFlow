"""Evaluate RAE (DiTDH) on ImageNetV2 (matched-frequency).
...
Follows the same phase structure as RAE's eval_imagenet.py:
    ...
    Phase 2: Gather to rank 0, load real ImageNetV2 references (shared dataloader)
...
Usage:
    ...
    torchrun --standalone --nproc_per_node=1 eval_imagenetv2.py \
        --config configs/stage2/sampling/<config>.yaml \
        --data_path /path/to/ImageNetV2 \
        --num_samples 5

    # Full run on 4 GPUs
    torchrun --standalone --nproc_per_node=4 eval_imagenetv2.py \
        --config configs/stage2/sampling/<config>.yaml \
        --data_path /path/to/ImageNetV2
"""
...
def generate_images(config_path, num_samples, batch_size, seed, precision,
    ...
    # ---- This rank's share comes from the contiguous v2 label slice ----
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate RAE on ImageNetV2 matched-frequency")
    parser.add_argument('--config', type=str, required=True,
                        help='Path to RAE sampling config YAML '
                             '(e.g., configs/stage2/sampling/...)')
    parser.add_argument('--data_path', type=str, required=True,
                        help='Path to ImageNetV2 root (contains imagenetv2-matched-frequency-format-val/)')
    ...
    subfolder_name = f'{tag}_steps_{_num_steps_early}_imagenetv2'
    ...
    # ---- Load v2 labels on all ranks and distribute contiguous slices ----
    _meta = ImageNetV2Dataset(data_path=args.data_path,
                              image_size=args.image_size, return_uint8=True)
