"""Evaluate a MeanFlow checkpoint on ImageNet.

Generates class-conditioned samples using CFG at inference, decodes latents
via VAE, and computes FID, Inception Score, CLIP Score, and PickScore against real ImageNet images.
...
# (e.g. "3,5,7"). --num_gpus trims that to the first N entries, filtering out
# GPUs that are already heavily occupied by other processes.
def _parse_num_gpus():
...
def _free_gpu_memory_mb():
    """Return {gpu_id: free_mib} using nvidia-smi (no CUDA init required)."""
    import subprocess
    try:
        ...
        return {}  # can't query; caller falls back to unfiltered list

def _select_gpus(allocated_ids, want_n, min_free_mb=4096):
    """Pick up to want_n GPUs from allocated_ids with >= min_free_mb MiB free.

    Falls back to the first want_n entries if nvidia-smi is unavailable.
    """
    mem = _free_gpu_memory_mb()
    if not mem:
        return allocated_ids[:want_n]
    usable = [g for g in allocated_ids if mem.get(int(g), 0) >= min_free_mb]
    if not usable:
        # All GPUs appear full; fall back rather than running on zero devices
        usable = sorted(allocated_ids, key=lambda g: mem.get(int(g), 0), reverse=True)
    return usable[:want_n]

_NUM_GPUS = _parse_num_gpus()
if _NUM_GPUS > 0:
    if 'CUDA_VISIBLE_DEVICES' in os.environ:
        # Cluster: pick N free GPUs from scheduler-allocated set
        allocated = os.environ['CUDA_VISIBLE_DEVICES'].split(',')
    else:
        # Local: consider all GPUs as candidates
        import subprocess
        try:
            ...
            n_total = _NUM_GPUS
        allocated = [str(i) for i in range(n_total)]
    selected = _select_gpus(allocated, _NUM_GPUS)
...
flags.DEFINE_integer('num_steps', 25, 'Number of sampling steps')
...
flags.DEFINE_integer('num_gpus', 1, 'Number of GPUs to use (0 = use all available)')
...
flags.DEFINE_string('output_dir', None,
                    'Override base output directory (default: outputs/meanflow/steps_{num_steps})')
...
def _save_best_worst_fid_grid(generated_images, all_labels, fid_per_class,
    ...
    Args:
        generated_images: uint8 NHWC numpy array of generated images.
        all_labels: int array, same length as generated_images; all_labels[i] is the
            class that generated_images[i] was conditioned on.
        fid_per_class: dict[int, float] mapping class_idx -> per-class FID.
    ...
    for row_idx, (group, color) in enumerate([(best10, '#1a9850'), (worst10, '#d73027')]):
        for col_idx, (cls, fid_val) in enumerate(group):
            ...
            # Find first generated image conditioned on this class
...
def evaluate(config, data_path, cfg_scale, use_ema, resume=False,
    ...
    if skip_gen:
        ...
        _meta = ImageNetDataset(data_path=data_path, split='val',
        ...
        if resume and os.path.exists(partial_latents_path):
            try:
                ...
                log_for_0(f'Partial latent checkpoint is corrupted, starting from scratch.')
                os.remove(partial_latents_path)
    ...
    # ==================================================================
    # Sanity-check: generated_images[i] was conditioned on all_labels[i].
    # ImageNetDataset iterates synsets in sorted order so val_labels_all is
    # class-sorted (class 0…999); the generation loop slices those labels
    # sequentially, and the checkpoint saves val_labels_all verbatim.
    assert len(all_labels) == len(generated_images), (
        f"Label/image count mismatch: {len(all_labels)} labels vs "
        f"{len(generated_images)} images"
    )
