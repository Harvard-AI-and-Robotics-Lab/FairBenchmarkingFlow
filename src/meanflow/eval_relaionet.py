"""Evaluate a MeanFlow checkpoint on reLAIONet (CLIP-filtered web-scraped ImageNet subset).

Generates class-conditioned samples using CFG at inference, decodes latents
via VAE, and computes FID, Inception Score, CLIP Score, and PickScore against
real reLAIONet images.
...
os.environ.setdefault('XLA_PYTHON_CLIENT_ALLOCATOR', 'platform')  # use cudaMalloc/cudaFree for clean teardown alongside PyTorch
...
FLAGS = flags.FLAGS
flags.DEFINE_string('data_path', None, 'Path to reLAIONet root (contains reLAIONet-cleaned-v1/)')
...
flags.DEFINE_boolean('test', False, '[unused] Always evaluates on the full reLAIONet set')
...
# ======================================================================
# CFG Generation (mirrors eval_imagenetv2.py pattern)
# ======================================================================
...
def sample_step_cfg(variable, sample_idx, labels, model, rng_init, device_batch_size,
    ...
    labels: (device_batch_size,) int32 — reLAIONet dataset class indices for this device.
    """
...
def _compute_all_metrics(generated_images, all_labels, real_images, class_names,
    ...
    if 'clip_score' not in results:
        ...
        clip_mean, clip_per_class = eval_metrics.compute_clip_score(
            generated_images, all_labels, class_names, batch_size=1, return_per_class=True
        )
...
def evaluate(config, data_path, cfg_scale, use_ema, resume=False,
    ...
    subfolder_name = f'{model_name}_steps_{num_steps}_relaionet'
    ...
    if skip_gen:
        ...
        # ---- Load reLAIONet labels — used to drive class-conditional generation ----
        _meta = ReLAIONetDataset(data_path=data_path,
        ...
        for step in range(start_step, num_gen_steps):
            ...
            # Slice reLAIONet labels for this step and distribute across local devices
        ...
        all_labels = val_labels_all  # ordered reLAIONet labels, no trimming needed
    ...
    log_for_0('Phase 3: Loading all real reLAIONet images for reference...')
...
def main(argv):
    ...
    # Evaluate on the full reLAIONet set
    ds = ReLAIONetDataset(data_path=FLAGS.data_path, return_uint8=True)
    num_samples = len(ds)
    del ds
    log_for_0(f'Evaluating with all {num_samples} reLAIONet samples')
    ...
    # Skip Python's destructor chain to avoid CUDA_ERROR_ILLEGAL_ADDRESS at
    # process exit. The error occurs because PyTorch's CUDA destructor destroys
    # the primary CUDA context before JAX/XLA can call cuEventDestroy on its
    # events. os._exit(0) bypasses all finalizers; all results are saved above.
    jax.clear_caches()
    os._exit(0)
