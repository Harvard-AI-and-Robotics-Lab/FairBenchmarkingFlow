"""Evaluate SD 3.5 Large on ImageNetV2 (matched-frequency).
...
Score, CLIP Score, PickScore, and ImageReward against real ImageNetV2
matched-frequency images.
...
Supports multi-GPU sharding:
    # GPU 0 of 4
    CUDA_VISIBLE_DEVICES=0 python eval_imagenetv2.py \
        --data_path /path/to/ImageNetV2 --shard_id 0 --num_shards 4 \
...
Usage:
    ...
    # Full ImageNetV2 benchmark (~10k images)
    python eval_imagenetv2.py \
        --data_path /path/to/ImageNetV2 \
        --num_steps 28
"""
...
def generate_images(num_samples, num_steps, guidance_scale, seed, batch_size,
    ...
    Args:
        ...
        val_labels: numpy array of integer class labels ordered by the ImageNetV2 dataset.
    ...
    with torch.no_grad():
        ...
        while num_generated < num_samples:
            ...
            # ImageNetV2 class labels (ordered by the dataset)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate SD 3.5 Large on ImageNetV2 matched-frequency")
    parser.add_argument('--data_path', type=str, required=True,
                        help='Path to ImageNetV2 root')
    ...
    if args.shard_id is not None:
        ...
        # Load class names and ImageNetV2 labels
    ...
    # ==================================================================
    print(f"SD 3.5 Large ImageNetV2 Evaluation")
    ...
    if args.resume:
        ...
        # Load class names and ImageNetV2 labels from the dataset
        ref_ds = ImageNetV2Dataset(data_path=args.data_path,
                                   image_size=args.image_size, return_uint8=True)
        class_names = ref_ds.class_names
        val_labels_all = np.array([s[1] for s in ref_ds.samples], dtype=np.int32)
    ...
    # ==================================================================
    # Phase 2: Load real ImageNetV2 reference images
    # ==================================================================
    print("=" * 60)
    ...
    subfolder_name = f'{tag}_imagenetv2'
