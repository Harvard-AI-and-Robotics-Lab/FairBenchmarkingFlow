"""Generate class-conditioned images using SD 3.5 Large.
...
--ckpt_path can be a HuggingFace model ID (e.g. "stabilityai/stable-diffusion-3.5-large")
or a path to a local directory containing the model weights.

Usage:
    python eval_generate.py \
        --ckpt_path stabilityai/stable-diffusion-3.5-large \
        --classname goldfish \
        --num_steps 28 \
        --cfg_scale 3.5 \
        --num_samples 4
...
def main():
    parser = argparse.ArgumentParser(description='Generate images with SD 3.5 Large')
    parser.add_argument('--ckpt_path', required=True,
                        help='HuggingFace model ID or local path '
                             '(e.g. stabilityai/stable-diffusion-3.5-large)')
    ...
    print(f"Loading SD 3.5 Large pipeline from '{args.ckpt_path}'...")
    pipe = StableDiffusion3Pipeline.from_pretrained(args.ckpt_path, torch_dtype=dtype)
