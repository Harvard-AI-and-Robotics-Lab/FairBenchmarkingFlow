"""
Evaluate SiT on ImageNet using shared benchmark utilities (ODE mode only).

Generates class-conditioned samples using all ImageNet val labels (in order),
decodes latents via VAE, and computes FID, Inception Score, CLIP Score, and
PickScore against real ImageNet val images.
...
def _load_all_val_images(ref_dataset, batch_size, num_workers):
    """Load all val images in dataset order as uint8 NHWC numpy array.
...
def _generate_samples_memmap(
    ...
    """Generate images conditioned on ordered val labels, writing to memmap cache."""
...
def _compute_all_metrics(generated_images, labels, real_images, class_names,
                         real_class_labels=None, compute_fid_IS_per_class=False,
                         cache_dir=None, batch_size=None, num_gpus=None):
    ...
    if batch_size is not None:
        ...
        is_kwargs['batch_size'] = batch_size
        clip_kwargs['batch_size'] = batch_size
        pick_kwargs['batch_size'] = batch_size
    if num_gpus is not None:
        fid_kwargs['num_gpus'] = num_gpus
        is_kwargs['num_gpus'] = num_gpus
        clip_kwargs['num_gpus'] = num_gpus
        pick_kwargs['num_gpus'] = num_gpus

    if 'fid' not in results:
        print('Computing FID...')
        results['fid'] = float(eval_metrics.compute_fid(real_images, generated_images, **fid_kwargs))
    ...
    if 'is_mean' not in results:
        ...
        is_mean, is_std = eval_metrics.compute_inception_score(generated_images, **is_kwargs)
    ...
    if compute_fid_IS_per_class and real_class_labels is not None:
        ...
        _expected_classes = set(int(c) for c in np.unique(labels))
        ...
        if not _expected_classes.issubset(_cached_fid_classes):
            ...
            fid_pc = eval_metrics.compute_fid_per_class(
                real_images, real_class_labels, generated_images, labels, **fid_kwargs
            )
        ...
        if not _expected_classes.issubset(_cached_is_classes):
            ...
            is_pc = eval_metrics.compute_inception_score_per_class(
                generated_images, labels, **is_kwargs
            )
    ...
    if 'clip_score' not in results:
        ...
        clip_mean, clip_per_class = eval_metrics.compute_clip_score(
            generated_images, labels, class_names, return_per_class=True, **clip_kwargs
        )
    ...
    if 'pick_score' not in results:
        ...
        pick_mean, pick_per_class = eval_metrics.compute_pick_score(
            generated_images, prompts, class_labels=labels, return_per_class=True, **pick_kwargs
        )
...
def main(args):
    ...
    print(f"[info] Dataset split='{args.split}', size={full_count}")

    print("[info] Loading all real ImageNet val images...")
    ...
    run_stem = f"{args.model_name}_{cfg_tag}_steps{steps}_imagenet"
    run_subdir = step_dir / run_stem
    run_subdir.mkdir(parents=True, exist_ok=True)
    ckpt_dir = run_subdir / "_checkpoints"
    ...
    print("\nSiT ImageNet benchmark run completed.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="SiT ImageNet benchmark (ODE mode) using src/utils/dataloader and src/utils/eval.py"
    )

    parser.add_argument("--data-path", type=str, default="~/autodl-tmp/imagenet")
