"""
Evaluate SiT on reLAIONet (CLIP-filtered web-scraped ImageNet subset) using shared benchmark utilities (ODE mode only).

Generates class-conditioned samples using all reLAIONet labels (in order),
decodes latents via VAE, and computes FID, Inception Score, CLIP Score, and
PickScore against real reLAIONet images.
...
def _load_all_val_images(ref_dataset, batch_size, num_workers):
    """Load all reLAIONet images in dataset order as uint8 NHWC numpy array."""
...
def _generate_samples_memmap(
    ...
    """Generate images conditioned on ordered reLAIONet labels, writing to memmap cache."""
...
def main(args):
    ...
    print(f"[info] reLAIONet dataset size={full_count}")

    print("[info] Loading all real reLAIONet images...")
    ...
    run_stem = f"{args.model_name}_{cfg_tag}_steps{steps}_relaionet"
    ...
    print("\nSiT reLAIONet benchmark run completed.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="SiT reLAIONet benchmark (ODE mode) using src/utils/dataloader and src/utils/eval.py"
    )

    parser.add_argument("--data-path", type=str, required=True,
                        help="Path to directory containing reLAIONet-cleaned-v1/")
