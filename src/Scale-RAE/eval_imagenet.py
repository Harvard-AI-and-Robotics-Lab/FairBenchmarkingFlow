"""Evaluate Scale-RAE on ImageNet.

Generates ImageNet class-conditioned samples using text prompts
...
ImageNet images using the shared evaluation utilities.
...
Follows the same phase structure as SD3.5-L/eval_imagenet.py:
...
# ---- Path setup ----
# Scale-RAE's own package lives in the same directory as this script.
# Shared utils (dataloader, eval) live one level up in src/.
...
def generate_images(model_path, decoder_path, num_samples, guidance_level,
    ...
    Generates one image at a time (the model is autoregressive and not
    trivially batchable). Uses tqdm for progress tracking and supports
    resume from a partial checkpoint.

    Args:
        model_path: HuggingFace model ID or local path to Scale-RAE LLM.
        decoder_path: Path to local RAE decoder .pt weights file.
        num_samples: Total number of images to generate.
        guidance_level: Guidance strength passed to model.generate().
        image_size: Target pixel resolution for output images.
        ...
        max_new_tokens: Max image tokens for autoregressive generation.
        ckpt_every: Save partial checkpoint every N images (0 = disable).
        output_dir: Directory to save partial checkpoint.
        device: torch device string (e.g. "cuda:0").
        num_steps: Number of diffusion inference steps for the DiT head.
                   If None or <=0, keeps the model default (50).

    Returns:
        generated_images: (N, H, W, 3) uint8 numpy array.
        all_labels: (N,) int64 numpy array of class indices.
    """
    # ---- Load model ----
    disable_torch_init()
    ...
    # ---- Val label sequence (ordered by the val dataset) ----
    ...
    with torch.inference_mode():
        for label in remaining_labels:
            ...
            if image_embeds is not None and image_embeds.ndim > 1:
                # image_embeds: [seq_len, hidden] -> [1, seq_len, hidden]
                image_embeds_3d = image_embeds.unsqueeze(0)

                # Prepend empty CLS token (required by decoder)
                ...
                # Decode latent features to pixel image
                with torch.no_grad():
                ...
                # Generation produced no image — use blank frame, log warning
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate Scale-RAE on ImageNet (FID, IS, CLIP Score, PickScore)"
    )
    ...
    print("Scale-RAE ImageNet Evaluation")
    ...
    tag = args.model_name
    subfolder_name = f'{tag}_steps_{args.num_steps}_imagenet'
    ...
    if args.resume:
        ...
        # If gen_only, stop here (other shards still running)
        if args.gen_only:
            print(f"Shard {args.shard_id} done. Exiting (--gen_only).")
            return

        # Single-shard mode: save as full checkpoint
        ckpt_path = ckpt_dir / 'generated_checkpoint.npz'
        np.savez_compressed(str(ckpt_path),
                            images=generated_images, labels=all_labels)
