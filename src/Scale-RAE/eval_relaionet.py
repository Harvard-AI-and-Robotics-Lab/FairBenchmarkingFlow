"""Evaluate Scale-RAE on reLAIONet (CLIP-filtered web-scraped ImageNet subset).

Generates reLAIONet class-conditioned samples using text prompts
...
def generate_images(model_path, decoder_path, num_samples, guidance_level,
    ...
    # ---- Label sequence (ordered by the reLAIONet dataset) ----
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate Scale-RAE on reLAIONet (FID, IS, CLIP Score, PickScore)"
    )
    ...
    print("Scale-RAE reLAIONet Evaluation")
    ...
    tag = args.model_name
    subfolder_name = f'{tag}_steps_{args.num_steps}_relaionet'
